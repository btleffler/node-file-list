...
File = function File (path, collector, index, stats) {
    ...
    function determineHidden () {
        var cmd = "cscript " + __dirname + "\\whf.js " + self.path + " //Nologo";
...
File.prototype.isHidden = function isHidden () {
    return this.name.charAt(0) === '.' || this.stats.windowsHidden;
};
...
FileCollector.prototype.getFiles = function getFiles () {
    var files = [],
        i = 0,
        len = this.files.length,
        file;

    if (this.app.get("showHiddenFiles"))
        return this.files;
